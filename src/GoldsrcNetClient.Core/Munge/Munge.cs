...
public static class MungeEngine
{
    ...
    /// <summary>
    /// One transform serving both directions. Per 32-bit word the engine applies
    /// key/byteswap/table steps; UnMunge is the same pipeline with the steps in
    /// reverse order. Note the byte swap sits on opposite sides of the table step
    /// (encrypt: swap before, decrypt: swap after) and must not commute with the
    /// XORs, since a byte permutation does not commute with a 32-bit XOR key.
    /// </summary>
    private static void Apply(byte[] data, int len, int seq, ReadOnlySpan<byte> table, bool encrypt)
    {
        Span<byte> sp = data.AsSpan(0, len);
        Span<byte> p = stackalloc byte[4];
        int words = (len & ~3) / 4;
        int outerKey = encrypt ? ~seq : seq;
        int innerKey = encrypt ? seq : ~seq;

        for (int i = 0; i < words; i++)
        {
            int offset = i * 4;
            int c = BinaryPrimitives.ReadInt32LittleEndian(sp[offset..]) ^ outerKey;
            if (encrypt)
    ...
    public static void Munge1(byte[] data, int len, int seq) => Apply(data, len, seq, _table1, encrypt: true);

    /// <summary>Decrypts data using UnMunge table 1 (connectionless packets).</summary>
    public static void UnMunge1(byte[] data, int len, int seq) => Apply(data, len, seq, _table1, encrypt: false);

    /// <summary>Encrypts data using Munge table 2 (connected packets).</summary>
    /// <param name="data">The byte array to munge in-place.</param>
    /// <param name="len">Number of bytes to process (rounded down to nearest 4).</param>
    /// <param name="seq">Sequence number used as the XOR key component.</param>
    public static void Munge2(byte[] data, int len, int seq) => Apply(data, len, seq, _table2, encrypt: true);

    /// <summary>Decrypts data using UnMunge table 2 (connected packets). Inverts <see cref="Munge2"/>.</summary>
    public static void UnMunge2(byte[] data, int len, int seq) => Apply(data, len, seq, _table2, encrypt: false);

    /// <summary>Encrypts data using Munge table 3 (worldmap CRC).</summary>
    /// <param name="data">The byte array to munge in-place.</param>
    /// <param name="len">Number of bytes to process (rounded down to nearest 4).</param>
    /// <param name="seq">Sequence number used as the XOR key component.</param>
    public static void Munge3(byte[] data, int len, int seq) => Apply(data, len, seq, _table3, encrypt: true);

    /// <summary>Decrypts data using UnMunge table 3 (worldmap CRC). Inverts <see cref="Munge3"/>.</summary>
    public static void UnMunge3(byte[] data, int len, int seq) => Apply(data, len, seq, _table3, encrypt: false);
}
